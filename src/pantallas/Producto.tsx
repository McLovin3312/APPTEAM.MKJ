...
export type Producto = { id: number; nombre: string; descripcion: string; valor_unitario: number; stock: number };
type Form = { nombre: string; descripcion: string; valor_unitario: string; stock: string };

const VACIO: Form = { nombre: "", descripcion: "", valor_unitario: "", stock: "" };
...
const Tarjeta = ({ item, onVer, onEditar, onEliminar }: TarjetaProps) => (
  <View style={s.card}>
    <View style={{ flex: 1 }}>
      ...
      <Text style={s.cardSub}>{item.descripcion}</Text>
      <Text style={s.cardSub}>{formatCOP(item.valor_unitario)} · Stock: {item.stock}</Text>
    </View>
    <View>
...
const ProductosScreen = ({ navigation }: { navigation: any }) => {
  ...
  const cargar = async () => {
    setCargando(true);
    const { data } = await supabase.from("producto").select("*").order("id");
  ...
  const validar = () => {
    ...
    if (!form.valor_unitario || isNaN(Number(form.valor_unitario)) || Number(form.valor_unitario) <= 0)
                                  e.valor_unitario = "Valor inválido";
  ...
  const abrirEditar = (p: Producto) => {
    setForm({ nombre: p.nombre, descripcion: p.descripcion, valor_unitario: String(p.valor_unitario), stock: String(p.stock) });
    setEditId(p.id); setErrores({}); setModal("form");
  };
  ...
  const guardar = async () => {
    ...
    if (editId) {
      await supabase.from("producto").update(payload).eq("id", editId);
    } else {
  ...
  const eliminar = async () => {
    setCargando(true);
    await supabase.from("producto").delete().eq("id", elimId);
    await cargar();
    ...
    <View style={s.container}>
      ...
      {cargando
        ...
        : productos.length === 0
          ...
          : <FlatList data={productos} keyExtractor={p => String(p.id)}
      ...
      <Modal visible={modal === "form"} transparent animationType="slide">
        <View style={s.overlay}>
          <View style={s.modal}>
            <Text style={s.modalTitle}>{editId ? "Editar producto" : "Nuevo producto"}</Text>
            <Campo label="Nombre"           value={form.nombre}         onChange={set("nombre")}         error={errores.nombre}         placeholder="Nombre" />
            <Campo label="Descripción"      value={form.descripcion}    onChange={set("descripcion")}    error={errores.descripcion}    placeholder="Descripción" />
            <Campo label="Valor Unit. (COP)" value={form.valor_unitario} onChange={set("valor_unitario")} error={errores.valor_unitario} placeholder="Ej: 15000" keyboardType="numeric" />
            <Campo label="Stock"            value={form.stock}          onChange={set("stock")}          error={errores.stock}          placeholder="Ej: 10" keyboardType="numeric" />
      ...
      <Modal visible={modal === "ver"} transparent animationType="fade">
        <View style={s.overlay}>
          <View style={s.modal}>
            <Text style={s.modalTitle}>Info del producto</Text>
            {detalle && <>
              <Fila label="ID"          valor={String(detalle.id)} />
              <Fila label="Nombre"      valor={detalle.nombre} />
              <Fila label="Descripción" valor={detalle.descripcion} />
              <Fila label="Valor Unit." valor={formatCOP(detalle.valor_unitario)} />
              <Fila label="Stock"       valor={String(detalle.stock)} />
            </>}
